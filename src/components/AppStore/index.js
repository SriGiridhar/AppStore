...
class AppStore extends Component {
  ...
  render() {
    ...
    const updatedResults = searchResults.filter(eachResult =>
      eachResult.appName.toLowerCase().includes(input.toLowerCase()),
    )
    return (
      <div className="bgContainer">
        <h1 className="AppStore">App Store</h1>
        ...
        <ul className="categoryItem">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              isActive={eachTab.tabId === category}
              eachTab={eachTab}
